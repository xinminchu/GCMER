...
vector<string> GraphColoring::Lmxrlf::make_independent_set() {
    ...
    while(todo.size()) {
        set.push_back(todo[Rcpp::sample(todo.size(), 1)[0] - 1]);
...
int GraphColoring::Lmxrlf::min_objf(vector< vector<string> > list_of_best) {
    if(list_of_best.size() == 0) {
        Rcerr << "List of Best Solutions is of size 0" << endl;
        return -1;
    }
    ...
    do {
        ...
        for(int i=0; i<global_iterations; i++) {
            ...
            while(uncolored_neighbor(set_t).size() == 0) {
                int removal = Rcpp::sample(set_t.size(), 1)[0] - 1;
            ...
            while(ucn.size() > 0) {
                set_t.push_back(ucn[Rcpp::sample(ucn.size(), 1)[0] - 1]);
